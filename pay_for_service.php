...
if (isset($_POST['service'])) {
    ...
    } elseif ($service === "school_fee" && !empty($_POST['school_fee_amount'])) {
        $amount = floatval($_POST['school_fee_amount']);
    ...
    $updateQuery = "UPDATE register SET wallet = '$newWalletBalance' WHERE username = '$username'";
    ...
    // Subtract the amount from the user's Airtime Wallet in the register table ---
    $updateQuery = "UPDATE register SET airtimeWallet = '$newairtimeWallet' WHERE username = '$username'";
    $updateResult = $conn->query($updateQuery);
